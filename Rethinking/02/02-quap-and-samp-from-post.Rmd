...
blood_quap <- quap(
  alist(
    ...
    obs ~ dbinom(n, p),  # binomial likelihood 
...
> The quadratic approximation perform more precise when I increase the dataset size, more sample = more information for the posterior.
...
> We can easily control the sample size, and we don't want to affect the original blood_data2.
...
```{r}
gf_histogram(~p_grid, data = samples)
gf_density(~p_grid, data = samples)
gf_violin(p_grid ~ "", data = samples)
```

### Question 7: What's the best number of posterior samples to get? How would you recommend choosing?

> I recommend around 2000, if it's too low the graph looks rough, if it's too high the graph looks like a chainsaw.
