...
> Prior is Uniform(0, 1)
>
> likelihood is Binomial(n, p)
...
> A person is normally distributed with a mean of 178 cm and standard deviation of 20 cm.
>
> The sigma is uniformly distributed between 0 and 50.

#### Question: What part tells us about the prior? What part about the likelihood?

> likelihood is $$\text{height}_i \sim \text{Normal}(\mu, \sigma)$$
>
> priors are
>
> $$\mu \sim \text{Normal}(\text{mean} = 178, \text{sd} = 20)$$
...
>
> Uniform distribution is chosen because we assume all values equally 0 and 50 equally likely.
>
> I would chose normal distribution for $\mu$, but I'm not sure about $\sigma$ is equally likely between 0 and 50, most people are within a range.
>
> I worry that uniform distribution for $\sigma$ is not accurate.

#### Question: How might you adjust the prior to better reflect your own knowledge?

> Maybe change the uniform distribution to (0,20).
...
> $$\text{water_expenses_pp}_i \sim \text{Normal}(\mu, \sigma)$$
>
> $$\mu \sim \text{Normal}(\text{mean} = 10, \text{sd} = 5)$$
>
...
-   Right now we still have *only one* data variable, so none of the causal diagram stuff comes into play yet...can you imagine how it *will* soon come into play?
    ...
    > We will have more variables like household income, number of adults/children... for more complex model?
