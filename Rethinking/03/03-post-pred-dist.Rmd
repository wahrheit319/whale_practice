...
-   So...how did we choose the `n` to be 10,000? Why is the `size` 9? And how was 0.71 picked as the `prob`? *Hint: there's a problem with just using 0.71!)*
    ...
    > n = 10000 because we want to have a large sample size taht is enough for the simulation.
    >
...
post_samples <- extract.samples(blood_quap, n = 10000)$p |>
  na.omit()
...
B_ppd_data <- tibble(value = B_ppd)

gf_histogram(~ value, data = tibble(value = B), fill = "red", binwidth = 1) |>
  gf_histogram(~ value, data = tibble(value = B_ppd), fill = "blue", binwidth = 1)
```
what is wrong?...
