...
#### Question: How does `B_ppd` (the posterior predictive distribution) differ from the `B` you generated earlier using `rbinom()`? You can just explain based on your understanding so far, or if you are not sure, try making graphs of `B` and `B_ppd` to help you figure it out.
...
B_ppd is standardized posterior sample with uncertanty, while B is just selecting 10000 samples doing size = 9, prob = 0.33. these parameters are fixed, without uncertainty.
